...
export class GraphRepresentationComponent implements OnInit {
  ...
  ReadJobTracks() {
    ...
    this.chartData.jobTrackList().subscribe((result) => {
      if (result) {
        ...
        this.chartData.trackname = result[0].trackName;
        console.log(this.chartData.trackname)
        //this.createChart1(0);
      }
    })
  }
  // createChart1(ind:number){
  //   if(this.jobTrack && this.n<this.jobTrack.length){
  //     this.n++;
  //     this.chartData.trackname=this.jobTrack[ind].trackName;
  //     this.summitRequirement=this.jobTrack[ind].summitRequirement
  //     this.pendngRequirement=this.jobTrack[ind].pendngRequirement
  //     this.CIRejected=this.jobTrack[ind].CIRejected
  //     this.CIScheduled=this.jobTrack[ind].CIScheduled
  //     this.fullfilledPositions=this.jobTrack[ind].fullfilledPositions
  //     this.CITBS=this.jobTrack[ind].CITBS
  ...
  //         datasets: [
  //           {
  //             label: this.jobTrack[ind].trackName,
  ...
  creategraph(ind: number) {
    ...
    this.chart1 = new Chart("MyCharts1", {
      type: 'bar', //this denotes the type of chart
